...
export class RequestObservabilityInterceptor implements NestInterceptor {
  private readonly baseLogger = new StructuredLogger("planning-service", new NestLogSink());
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<unknown> {
    ...
    const requestId = (
      RequestIdSchema.safeParse(suppliedRequestId).success ? suppliedRequestId : randomUUID()
    ) as RequestId;
